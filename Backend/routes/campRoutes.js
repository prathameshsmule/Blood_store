...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    const camp = await Camp.create({ name, location, date, organizerName, organizerContact, proName, hospitalName })
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (payload.name) {
      ...
      if (sameName) return res.status(409).json({ message: 'Another camp with this name already exists' })
    }
